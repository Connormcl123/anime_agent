...
def discover_favorite_scenes(book_name):
    ...
    prompt = f"""
    Analyze the book '{book_name}' and output only valid JSON:
    {{
      "scenes": [
        {{
          "scene_number": 1,
    ...
    if r.status_code == 200:
        ...
        try:
            ...
            print("[ERROR] Invalid JSON raw saved.")
...
def load_favorite_scenes():
    ...
    if isinstance(data, dict) and "scenes" in data:
        ...
        print("[ERROR] Invalid format in favorite_scenes.json")
        sys.exit(1)
...
def anime_summarize(scene, book_context=None):
    ...
    prompt = f"""
    Write an anime-style screenplay matching the scene description.
...
def generate_anime_frame(scene_visual, output_filename):
    ...
    print(f"[IMG] Generating frame for: {scene_visual}")
    r = requests.post(url, headers=headers, json=payload)
    if r.status_code == 200:
        try:
            result = r.json()
        ...
        print(f"[ERROR] Azure Image failed: {r.status_code} {r.text}")
...
def svd_all_frames_to_videos():
    ...
    for frame_path in sorted(glob.glob(os.path.join(frames_dir, "*.png"))):
        ...
        svd_image_to_video_replicate(frame_path, video_path)
...
def run_full_workflow():
    ...
    if anime_json:
        ...
        for idx, scene_data in enumerate(anime_json["scenes"], start=1):
            frame_file = os.path.join(frames_dir, f"scene_{idx}.png")
            generate_anime_frame(scene_data["visual"], frame_file)
            print("[WAIT] Pausing to respect Azure rate limits...")
            time.sleep(25)
        # Replicate SVD Videos
...
def run_video_generation_only():
    print("[RUN] Video generation-only mode...")
    svd_all_frames_to_videos()
